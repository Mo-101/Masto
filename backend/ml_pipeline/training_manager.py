...
from shared.synthetic_data import integrate_mostlyai_data, validate_synthetic_data
...
class TrainingManager:
    ...
    async def augment_training_data(self, training_data: pd.DataFrame, data_type: str) -> pd.DataFrame:
        """
        Augment training data with MostlyAI synthetic data.
        
        Args:
            training_data: Original training data
            data_type: Type of data being augmented
        
        Returns:
            Augmented DataFrame with synthetic data
        """
        try:
            logger.info(f"Augmenting {data_type} training data with MostlyAI synthetic data")
            
            # Generate synthetic data using MostlyAI
            synthetic_records = await integrate_mostlyai_data()
            
            # Validate synthetic data quality
            validation_results = await validate_synthetic_data(synthetic_records)
            logger.info(f"Synthetic data validation: {validation_results}")
            
            if validation_results.get('overall_quality_score', 0) < 0.8:
                logger.warning("Synthetic data quality below threshold, using original data only")
            ...
            common_columns = set(training_data.columns) & set(synthetic_df.columns)
            if common_columns:
                synthetic_df = synthetic_df[list(common_columns)]
                augmented_data = pd.concat([training_data, synthetic_df], ignore_index=True)
                logger.info(f"Augmented training data: {len(training_data)} original + {len(synthetic_df)} synthetic = {len(augmented_data)} total")
    ...
    async def train_model(self, model_type: str) -> Dict:
        ...
        try:
            ...
            training_data = await self.augment_training_data(training_data, model_type)
